...
def manager_required(f):
    """Decorator: allows admin and manager roles only."""
    @wraps(f)
    def decorated(*args, **kwargs):
        if 'user_id' not in session:
            return redirect(url_for('login'))
...
def admin_delete_user(user_id):
    ...
    # ── Permanently delete all data linked to this user ───────────────────────
    # 1. Unassign any jobs this technician/manager was assigned to
    db.execute("UPDATE jobs SET assigned_tech_id=NULL WHERE assigned_tech_id=?", (user_id,))
    # 2. Remove their entries from job_logs
    db.execute("DELETE FROM job_logs WHERE performed_by=?", (user_id,))
    # 3. Remove any notifications sent to them
    db.execute("DELETE FROM notifications WHERE recipient_id=?", (user_id,))
    # 4. Finally delete the user row itself
    db.execute("DELETE FROM users WHERE id=?", (user_id,))
    db.commit()

    flash(f'✅ User "{user["name"]}" and all their data permanently deleted.', 'success')
    return redirect(url_for('admin_users'))
